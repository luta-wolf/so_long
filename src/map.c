...
/*   Updated: 2022/01/26 14:08:16 by einterdi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	all.img.player = mlx_xpm_file_to_image(all.mlx, PLAYER, &all.img.sprite_x, &all.img.sprite_y);
	all.img.coin = mlx_xpm_file_to_image(all.mlx, COIN1, &all.img.sprite_x, &all.img.sprite_y);
	all.img.exit = mlx_xpm_file_to_image(all.mlx, EXIT, &all.img.sprite_x, &all.img.sprite_y);
	if (!all.img.floor)
	{
		printf("(1)\n");
		exit(0);
	...
	mlx_put_image_to_window(all.mlx, all.win_mlx, all.img.floor, 0 + 1 * 64, 0 + 4 * 64);
	mlx_put_image_to_window(all.mlx, all.win_mlx, all.img.player, 0 + 2 * 64, 0 + 4 * 64);
	mlx_put_image_to_window(all.mlx, all.win_mlx, all.img.coin, 0 + 3 * 64, 0 + 4 * 64);
	mlx_put_image_to_window(all.mlx, all.win_mlx, all.img.exit, 0 + 4 * 64, 0 + 4 * 64);
